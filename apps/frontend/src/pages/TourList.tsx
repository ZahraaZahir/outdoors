...
export default function TourList() {
  ...
  const filtered = useMemo(() => {
    return tours.filter((tour) => {
      ...
      const matchesDate =
        !dateFilter ||
        new Date(tour.date).toLocaleDateString('sv-SE') === dateFilter;
      return matchesSearch && matchesDate;
    });
  }, [tours, search, dateFilter]);

  const handlePrefetch = (tourId: number) => {
        ...
        <div className="relative mx-auto max-w-6xl px-4 lg:px-6">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-lg text-primary-200">
              Discover dream destinations, plan trips with like-minded
              adventurers. Outdoor experiences that create real connections.
            </p>
          </div>
            ...
            <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((tour) => (
                <Link
                  ...
                  onMouseEnter={() => handlePrefetch(tour.id)}
                >
                  <div className="relative h-56 overflow-hidden bg-gradient-to-br from-primary-600 to-primary-800">
                    {tour.imageUrl ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <svg
                          className="h-20 w-20 text-white/20 transition-transform duration-500 group-hover:scale-110"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={1}
                    ...
                    <div className="absolute bottom-4 left-4 right-4">
                      <div className="flex items-center gap-1.5 text-white/90">
                        ...
                        <span className="text-sm font-medium">
                          {getLocationShort(tour.destination)}
                        </span>
                      </div>
                    </div>
                  </div>
